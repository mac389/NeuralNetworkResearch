...
\begin{document}
	...
	\begin{abstract}
		...
		\mycomment{Its is different from it's.}

		\mycomment{The fundamental statement is: \emph{Neural networks can approximate any function.}. The activation function does not have to be sigmoidal. If a group of units uses sinusoids as their activation function, then the number of units needed to approximate the function to some precision will be the minimum possible.}

		
		General artificial intelligence refers to machine intelligence that performs a task as successfully as a human does. 
		
		\mycomment{$\Rightarrow$ The previous sentence does not make sense. How is \emph{general} AI defined in terms of the performance of \emph{one} task? }
...
\mycomment{I do not understand the words you wrote.}

\mycomment{Consider replacing what you wrote with the following: Here we construct a network that reports whether the network is adding or substracting two numbers (Figure \ref{fig:self-aware})}. 
...
\begin{figure}[b]
	...
	\node[bias] (operator-1) {$\oplus$};
	\node[input, below left = of operator-1] (operand-1) {$x_1$};
	\node[input, below right= of operator-1] (operand-2) {$x_2$};	
	
	\node[bias, below left= of operand-2] (operator-2) {$\ominus$};

	\node[input, above = of operator-1] (output-operator-1) {$x_1 \oplus x_2$};

	\node[input, above right = of operand-2] (operator-1-reporter) {$r_\oplus$};

	\draw[->, ultra thick] (operand-1) -- (operator-1);
	\draw[->, ultra thick] (operand-2) -- (operator-1);
	\draw[->,ultra thick] (operator-1) -- (output-operator-1);

	\draw[->] (operator-1) -- (operator-1-reporter);

	\node[input, below = of operator-2] (output-operator-2) {$x_1 \ominus x_2$};

	\node[input, below right = of operand-2] (operator-2-reporter) {$r_\ominus$};

	\draw[->, ultra thick] (operand-1) -- (operator-2);
	\draw[->, ultra thick] (operand-2) -- (operator-2);
	\draw[->,ultra thick] (operator-2) -- (output-operator-2);

	\draw[->] (operator-2) -- (operator-2-reporter);

	\end{tikzpicture}
	\caption{\textbf{Network that reports which of two operands are active}.\label{fig:self-aware}}
\end{figure}
